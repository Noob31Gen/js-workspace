...
export function buildOnMessageHandler(): string {
  ...
  lines.push('      ' + JSON.stringify('        __autoResults__[fnName] = await fn(__workspace_args__);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      } else {') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('          var callArgs = paramNames.map(function(k) { return __workspace_args__[k]; });') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          __autoResults__[fnName] = await fn.apply(null, callArgs);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        } else {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          __autoResults__[fnName] = await fn(__workspace_args__);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      }') + ' + ' + NL + ' +');
