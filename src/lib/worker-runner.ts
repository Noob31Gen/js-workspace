...
export function buildOnMessageHandler(): string {
  ...
  lines.push('      ' + JSON.stringify('}') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('function __safeParseJson(val) {') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('        try { var fixed = val.replace(/([a-zA-Z0-9_$]+)\\s*:/g, \'"$1":\').replace(/\'/g, \'"\'); return JSON.parse(fixed); } catch(e2) { return val; }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('  }') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('}') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('if (__workspace_args__ && typeof __workspace_args__ === "object") {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('  Object.keys(__workspace_args__).forEach(function(k) { __workspace_args__[k] = __safeParseJson(__workspace_args__[k]); });') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('}') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('    var fn = eval(fnName);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    if (typeof fn === "function") {') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('          var callArgs = paramNames.map(function(k) { return __safeParseJson(__workspace_args__[k]); });') + ' + ' + NL + ' +');
