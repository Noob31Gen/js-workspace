...
function buildOnMessageHandler(): string {
  ...
  lines.push('');
  lines.push("    var scriptFunc = new Function('__workspace_args__', 'require', 'workspace', 'process', 'Buffer', '__dirname', '__filename', 'dirname', 'filename',");
  ...
  lines.push('  );');
  lines.push('');
  lines.push('    var rawResult = await scriptFunc(args, self.require, self.workspace, self.process, self.Buffer, __dirname, __filename, dirname, filename);');
  lines.push('');
