...
export function buildOnMessageHandler(): string {
  ...
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    if (/get\\s+[a-zA-Z0-9_$]+\\s*\\(|set\\s+[a-zA-Z0-9_$]+\\s*\\(/.test(t)) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      try { var evalObj = new Function("return (" + t + ");"); return evalObj(); } catch(e) {}') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('          try { var evalObj = new Function("return (" + t + ");"); return evalObj(); } catch(e3) {}') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    if (/^\\/.*\\/[gimsuy]*$/.test(t)) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      try { var evalFunc = new Function("return (" + t + ");"); return evalFunc(); } catch(e) {}') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    if (/^(?:\\(|async\\s+function|class\\b|\\([a-zA-Z0-9_$,\\s]*\\)\\s*=>|function\\b|Symbol\\b|Promise\\b|Intl\\b|sanitizeQuery|new\\s+)/.test(t)) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      try { var evalFunc = new Function("Buffer", "sanitizeQuery", "return (" + t + ");"); var b = typeof Buffer !== "undefined" ? Buffer : (typeof self !== "undefined" && self.Buffer ? self.Buffer : null); var sq = typeof sanitizeQuery === "function" ? sanitizeQuery : null; return evalFunc(b, sq); } catch(e) {}') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('  }') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('      var destrKeys = [];') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      var destrMatch = fnStr.match(/^[^(]*\\(\\s*\\{([^}]*)\\}/);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      if (destrMatch && destrMatch[1]) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        var rawDestr = destrMatch[1].split(",");') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        for (var d = 0; d < rawDestr.length; d++) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          var dKey = rawDestr[d].split("=")[0].split(":")[0].trim().replace(/^\\.\\.\\./, "");') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          if (dKey) destrKeys.push(dKey);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      var destrArgs = {};') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      var hasCustomArg = false;') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      if (__workspace_args__ && typeof __workspace_args__ === "object") {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        for (var dk = 0; dk < destrKeys.length; dk++) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          var kName = destrKeys[dk];') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          if (__workspace_args__[kName] !== undefined && __workspace_args__[kName] !== "") {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('            destrArgs[kName] = __safeEvalValue(__workspace_args__[kName]);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('            hasCustomArg = true;') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('          }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      if (hasCustomArg) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        res = await fn(destrArgs);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      } else {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('        res = await fn();') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('      }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    } else {') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('        var callArgs = paramNames.map(function(k) { return __workspace_args__[k] !== undefined ? __safeEvalValue(__workspace_args__[k]) : undefined; });') + ' + ' + NL + ' +');
  ...
  lines.push('      ' + JSON.stringify('    }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('  } catch(e) {') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('    if (fnName !== "sanitizeQuery") console.error("Error executing " + fnName + ":", e.message);') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('  }') + ' + ' + NL + ' +');
  lines.push('      ' + JSON.stringify('}') + ' + ' + NL + ' +');
